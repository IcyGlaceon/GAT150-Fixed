...
void cool::PlayerComponent::Initialize(){
...
void cool::PlayerComponent::Update(){
	Vector2 direction = Vector2::zero;
	
	auto camera = m_owner->GetScene()->GetActorFromName("Camera");
	if (camera)
	{
		camera->m_transform.position = cool::Lerp(camera -> m_transform.position, m_owner->m_transform.position, 2 * g_time.deltaTime);
	}
	...
	if (m_groundCount > 0 && g_inputSystem.GetKeyState(key_space) == InputSystem::State::Press){
		...
		auto component = m_owner->GetComponent<PhysicsComponent>();
		if (component) {
			Vector2 force = Vector2::Rotate({ 1, 0 }, cool::DegToRad(m_owner->m_transform.rotation)) * thrust;
...
bool cool::PlayerComponent::Write(const rapidjson::Value& value) const{

	return true;
}

bool cool::PlayerComponent::Read(const rapidjson::Value& value){
...
void cool::PlayerComponent::OnNotify(const Event& event){
...
void cool::PlayerComponent::OnCollisionEnter(Actor* other){
...
void cool::PlayerComponent::OnCollisionExit(Actor* other){
	if (other->GetTag() == "Ground")
	{
		m_groundCount--;
	}
